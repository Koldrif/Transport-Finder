...
def update_license_and_bus(database):
    with open('log_license_and_bus_parsers.txt', 'w') as log:
        print('Start')
...
def update_prosecutor_check(database):
    with open('log_prosecutors_check_parsers.txt', 'w') as log:
        print('Start')
...
def update_category_register(database):
    with open('log_category_registry.txt', 'w') as log:
        print('Start')
...
if __name__ == '__main__':
    database = Database(host='127.0.0.1', port=3306, user='Ariken', password='F0ll0wAriken', db='transportfinder')
