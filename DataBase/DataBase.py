...
class DataBase:
    ...
    def __insert_database(self, license_number=None, inn=None, ogrn=None, vin=None, srm=None, **data):
...
def __insert_owner(self, inn='Н/Д', title='Н/Д', registred_at='Н/Д', license_number='Н/Д', reg_address='Н/Д',
                   implement_address='Н/Д', risk_category='Н/Д', **trash):
    request ="INSERT INTO transportfinder.transport " \
             "(VIN, State_Registr_Mark, Region, Date_of_issue, pass_ser, Ownership, brand, type, Registred_at, License number) " \
             "VALUES" \
             "('{VIN}', '{SRM}', '{Region}', '{DateOfIssue}', '{Serial}', '{Ownership}', '{Brand}', '{Type}', '{Registred_at}', '{License_number}');"
    with self.connect:
        cursor = self.connect.cursor()
...
def read_license_22_vologodsk(self):
    ...
    license_start_date = ':'.join(
        map(str, xldate(self.row[5], self.book.datemode)[:3:]))  # Дата начала действия лицензии
    status = self.row[6]
...
def read_license_25(self):
    ...
    order_date = ':'.join(map(str, xldate(self.row[12], self.book.datemode)[
                                   :3:]))  # Дата приказа (распоряжения) лицензирующего органа о предоставлении лицензии
